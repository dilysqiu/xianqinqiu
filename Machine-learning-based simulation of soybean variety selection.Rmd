...
ag <- read_excel("C:/Users/haoch/Desktop/DAT 500S Final Project/Training Data for Ag Project.xlsx")
eva_ag <- read_excel("C:/Users/haoch/Desktop/DAT 500S Final Project/Evaluation dataset for Ag Project.xlsx")
```
...
write.csv(yield_sd,"C:/Users/haoch/Desktop/DAT 500S Final Project/sd.csv", row.names = FALSE)
write.csv(variety_yield,"C:/Users/haoch/Desktop/DAT 500S Final Project/yield.csv", row.names = FALSE)
...
eva_vars <- colnames(eva_ag)
ds <- ag[ag$Variety == "V102", c("Variety_Yield", "Variety", eva_vars)]
str(ds)
```
...
set.seed(1)
train <- sample(1:nrow(ds), nrow(ds)/2)
ds.test <- ds[-train, "Variety_Yield"]

f = as.formula(paste('Variety_Yield ~', 
                     paste(names(ds)[!names(ds) %in% c("Variety_Yield")], collapse = '+')))
...
```{r}
library(MASS)
ds.fit <- lm(f, data = ds, subset = train)
lm.pred <- predict(ds.fit, newdata = ds[-train,])
mean((lm.pred - ds.test)^2)
```
